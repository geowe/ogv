...
export class FileLoader {
    ...
    beforeLoad() {
        if (this._totalFiles > 0) {
            ...
            this.loadProcessId = setInterval(() => {
                if (this._totalFiles === this._totalFileLoaded) {
                    this.finishLoad();
                }
            }, 1000);
        }
    ...
    finishLoad() {
        ...
        // if (this._mapSetting.mapScreenshot) alert('screenshot');
    }
    ...
    removeEmptyFeatures(fc) {
        ...
        if (removedFeatures !== 0) {
            alert('Se han detectado ' + removedFeatures + ' elementos sin geometría.');
        }
    ...
    calculateExtent(fc) {
        ...
        for (var i = 0; i < fc.length; i++) {
            extend(this._extent, fc[i].getGeometry().getExtent());
        }
    }
    ...
    setMapZoom(zoomSetting) {
        if (zoomSetting.value !== null) {
            this._map.getView().setZoom(zoomSetting.value);
        }
    }

    configureColorPalette(colorPaletteSetting) {
        if (colorPaletteSetting.color1 === null) {
            colorPaletteGenerator.setIncFactor(10);
        }

        colorPaletteGenerator.setAlpha(colorPaletteSetting.alpha);
        colorPaletteGenerator.setColors(
            colorPaletteSetting.color1,
            colorPaletteSetting.color2,
            colorPaletteSetting.colorTotal + 1,
            colorPaletteSetting.keepIndex
        );
    }
