...
export class AddLayerTool extends VisualElement {
    constructor(mapSetting) {
        ...
        super(document.getElementById('addLayerButtonContainer'), html);
        if (mapSetting.getSetting().addLayer) {
            this.configure(mapSetting);
        }
    }
    ...
    configure(mapSetting) {
        ...
        this.fileLoadPanel = new FileLoadPanel({
            parent: document.getElementById('fileLoadPanelContainer'), // document.body, // mapSetting.getSetting().map.getOverlayContainerStopEvent(),
