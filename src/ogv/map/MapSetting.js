...
export class MapSetting {
    constructor() {
        const loadStrategyType = urlParser.has(Parameter.LOAD_TYPE) ?
            urlParser.get(Parameter.LOAD_TYPE) :
            Parameter.INC_LOAD_TYPE;
        ...
        this._setting = {
            ...
            loader: {
                ...
                incrementalCount: urlParser.has(Parameter.LOAD_BLOCK_INC) ?
                    urlParser.get(Parameter.LOAD_BLOCK_INC) :
                    INCREMENTAL_COUNT_DEFAULT,
            },
            title: {
    ...
    getTitleValue() {
        ...
        if (urlParser.has(Parameter.TITLE_SIZE)) {
            size = `size=${urlParser.get(Parameter.TITLE_SIZE)}`;
        }

        if (urlParser.has(Parameter.TITLE_COLOR)) {
